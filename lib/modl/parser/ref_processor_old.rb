  ...
  module Parser
    ...
    class RefProcessorOld
      ...
      def split_by_ref_tokens str, global
        ...
        # Handle nested refs
        if parts[1].include? NESTED_SEPARATOR
          ref_key = Sutil.until(parts[1], NESTED_SEPARATOR)

          # recurse through the nested refs to a final value.
          num_ref_key = ref_key.to_i
          after_gt = Sutil.after(parts[1], NESTED_SEPARATOR)
          new_value, method_str = num_ref_key.to_s == ref_key ? nested_value(after_gt, global.index[num_ref_key], global) : nested_value(after_gt, global.pairs[ref_key], global)
          ...
          if new_value
            ...
            if new_value.is_a?(Fixnum)
              ...
              parts[1] = new_value
              parts[2] = Sutil.after(tmp, ref_key + method_str)
            else
      ...
      def nested_value ref, value, global
        ...
        if ref.include?(NESTED_SEPARATOR)
          end_index = ref.index('`')
          end_index = ref.length if end_index.nil?

          gt_index = ref.index(NESTED_SEPARATOR)
          ...
          if value.is_a? Parsed::ParsedPair
            ...
            num_ref = ref.to_i
            return value.array.abstractArrayItems[num_ref] if value.array
            ...
            return value.arrayValueItem.text + NESTED_SEPARATOR + ref
        ...
        elsif (value.is_a? Parsed::ParsedArrayValueItem || value.is_a?(Parsed::ParsedValueItem)) && value.string
          return value.extract_hash
        ...
        [value.text, NESTED_SEPARATOR + ref]
      end
